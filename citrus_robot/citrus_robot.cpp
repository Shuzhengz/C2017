...
CitrusRobot::CitrusRobot()
    : throttle_{1, c2017::QueueManager::GetInstance()->throttle_status_queue()},
      ...
      gamepad_{2,
               c2017::QueueManager::GetInstance()->manipulator_status_queue()},
      ds_sender_{c2017::QueueManager::GetInstance()->driver_station_queue()},
      ds_reader_{c2017::QueueManager::GetInstance()
...
void CitrusRobot::Update() {
  ...
  if (maybe_ds_status) {
    ...
    if (ds_status->mode() == RobotMode::AUTONOMOUS) {
      // lemonscript_.Start();  // Weird to call Start in a loop, but it's a
      // setter so it's fine
      ...
      auto superstructure_status = c2017::QueueManager::GetInstance()
                                       ->superstructure_status_queue()
                                       ->ReadLastMessage();
      if (superstructure_status) {
        muan::teleop::XBoxRumbleProto xbox_rumble;
        xbox_rumble->set_rumble(superstructure_status.value()->rumble_on() ? 1
                                                                           : 0);
        c2017::QueueManager::GetInstance()->xbox_rumble_queue()->WriteMessage(
            xbox_rumble);
      }
...
void CitrusRobot::SendSuperstructureMessage() {
  if (ball_intake_fast_->is_pressed()) {
    ...
    intake_group_goal_->set_ground_ball_rollers(
        intake_group::GROUND_BALL_IN_SLOW);
  ...
  } else if (operator_score_ground_gear_->is_pressed() ||
             driver_score_ground_gear_->is_pressed()) {
  ...
  if (drop_balls_->is_pressed()) {
    intake_group_goal_->set_ground_gear_intake(
        intake_group::GROUND_GEAR_START_DROPPING_BALLS);
  }

  if (drop_balls_->was_released()) {
    intake_group_goal_->set_ground_gear_intake(
        intake_group::GROUND_GEAR_STOP_DROPPING_BALLS);
  }

  intake_group_goal_->set_ground_ball_position(
      ball_intake_down_ ? intake_group::GROUND_BALL_DOWN
  ...
  c2017::QueueManager::GetInstance()->intake_group_goal_queue()->WriteMessage(
      intake_group_goal_);
  c2017::QueueManager::GetInstance()->shooter_group_goal_queue()->WriteMessage(
      shooter_group_goal_);
}
...
void CitrusRobot::SendDrivetrainMessage() {
  ...
  auto vision_status = c2017::QueueManager::GetInstance()
                           ->vision_status_queue()
                           ->ReadLastMessage();
  if (vision_status) {
  ...
  if (!using_vision_ || !vision_status || vision_status.value()->aligned()) {
    using_vision_ = false;
    c2017::QueueManager::GetInstance()->drivetrain_goal_queue()->WriteMessage(
        drivetrain_goal);
  }
  c2017::QueueManager::GetInstance()->vision_goal_queue()->WriteMessage(
      vision_goal);
}
