...
VisionAlignment::VisionAlignment()
    : should_align_{false},
      ...
      running_{false},
      properties_{7.0, 5.0, 3.0, 2.0,
                  c2017::drivetrain::GetDrivetrainConfig().robot_radius},
      vision_input_reader_{
          QueueManager::GetInstance()->vision_input_queue()->MakeReader()},
      driverstation_reader_{
          QueueManager::GetInstance()->driver_station_queue()->MakeReader()},
      dt_goal_queue_{
          c2017::QueueManager::GetInstance()->drivetrain_goal_queue()},
      dt_status_reader_{c2017::QueueManager::GetInstance()
                            ->drivetrain_status_queue()
                            ->MakeReader()},
      action_{muan::actions::DrivetrainAction(
          properties_,
          c2017::QueueManager::GetInstance()->drivetrain_goal_queue(),
...
void VisionAlignment::Update() {
  ...
  bool disabled = ds ? ((*ds)->mode() == RobotMode::DISABLED ||
                        (*ds)->mode() == RobotMode::ESTOP)
                     : true;

  auto goal =
      QueueManager::GetInstance()->vision_goal_queue()->ReadLastMessage();
  if (goal) {
  ...
  double target_distance =
      status->distance_to_target() - constants::kShotDistance;
  ...
  if (auto dt_status_message = dt_status_reader_.ReadLastMessage()) {
    terminated = terminated &&
                 std::abs(dt_status_message.value()->forward_velocity()) < 0.05;
  }
  ...
  if (!disabled && should_align_) {
    if (!running_) {
      ...
      params.termination =
          muan::actions::DrivetrainTermination{0.05, 0.05, 0.05, 0.05};
