...
class SuperstructureTest : public ::testing::Test {
 public:
  ...
  void WriteQueues() {
    QueueManager::GetInstance()->intake_group_goal_queue()->WriteMessage(
        intake_group_goal_proto_);
    QueueManager::GetInstance()->shooter_group_goal_queue()->WriteMessage(
        shooter_group_goal_proto_);
    QueueManager::GetInstance()->driver_station_queue()->WriteMessage(
        driver_station_proto_);
    QueueManager::GetInstance()->climber_input_queue()->WriteMessage(
        climber_input_proto_);
    QueueManager::GetInstance()->shooter_input_queue()->WriteMessage(
        shooter_input_proto_);
    QueueManager::GetInstance()->ground_gear_input_queue()->WriteMessage(
        ground_gear_input_proto_);
  }
...
TEST_F(SuperstructureTest, SysInactive) {
  ...
  intake_group_goal_proto_->set_ground_gear_intake(
      intake_group::GROUND_GEAR_DROP);
  intake_group_goal_proto_->set_ground_ball_rollers(
      intake_group::GROUND_BALL_IN);
  ...
  auto superstructure_output = QueueManager::GetInstance()
  ...
  auto superstructure_status = QueueManager::GetInstance()
...
TEST_F(SuperstructureTest, GearFunctionality) {
...

  {
    ...
    EXPECT_EQ(ground_gear_status->current_state(),
              c2017::ground_gear_intake::INTAKING);
  }

  intake_group_goal_proto_->set_ground_gear_intake(
      intake_group::GROUND_GEAR_NONE);
...

  {
    ...
    EXPECT_FALSE(superstructure_output->ground_gear_down());
    EXPECT_EQ(ground_gear_status->current_state(),
              c2017::ground_gear_intake::PICKING_UP);
  }
...

  {
    ...
    EXPECT_FALSE(superstructure_output->ground_gear_down());
    EXPECT_EQ(ground_gear_status->current_state(),
              c2017::ground_gear_intake::CARRYING);
  }
}
...
TEST_F(SuperstructureTest, BothIntakesStayInBox) {
  ...
  intake_group_goal_proto_->set_ground_gear_intake(
      intake_group::GROUND_GEAR_DROP);
  intake_group_goal_proto_->set_ground_ball_position(
...
TEST_F(SuperstructureTest, GearScoreStaysInBox) {
  ...
  intake_group_goal_proto_->set_ground_gear_intake(
      intake_group::GROUND_GEAR_SCORE);
  intake_group_goal_proto_->set_ground_ball_position(
      intake_group::GROUND_BALL_DOWN);
...
TEST_F(SuperstructureTest, SpinupShoot) {
  ...
  for (size_t i = 0; i < 1000; i++) {
    ...
    shooter_input_proto_->set_shooter_encoder_position(current_position);
    shooter_input_proto_->set_accelerator_encoder_position(current_position *
                                                           0.5);
    WriteQueues();
    superstructure.Update();
  }
  ...
  {
    auto shooter_status = QueueManager::GetInstance()
...
TEST_F(SuperstructureTest, Climb) {
...

  {
    ...
    EXPECT_EQ(superstructure_output->ground_gear_voltage(),
              c2017::ground_gear_intake::kOuttakeVoltage);
  }
}
