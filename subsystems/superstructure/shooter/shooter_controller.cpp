...
ShooterController::ShooterController()
  ...
  auto shooter_plant = muan::control::StateSpacePlant<1, 3, 1>(
  ...
  shooter_controller_ = muan::control::StateSpaceController<1, 3, 1>(
      frc1678::shooter_controller::controller::K(),
      frc1678::shooter_controller::controller::Kff(),
      frc1678::shooter_controller::controller::A(),
  ...
  shooter_observer_ = muan::control::StateSpaceObserver<1, 3, 1>(
      shooter_plant, frc1678::shooter_controller::controller::L());

  accelerator_controller_ = muan::control::StateSpaceController<1, 2, 1>(
      frc1678::accelerator_controller::controller::K(),
      frc1678::accelerator_controller::controller::Kff(),
  ...
  auto accelerator_plant = muan::control::StateSpacePlant<1, 2, 1>(
      frc1678::accelerator_controller::controller::A(),
      frc1678::accelerator_controller::controller::B(),
...
c2017::shooter::ShooterOutputProto ShooterController::Update(
  ...
  auto shooter_y =
      (Eigen::Matrix<double, 1, 1>() << input->shooter_encoder_position())
          .finished();
  shooter_r_ =
      (Eigen::Matrix<double, 3, 1>() << 0.0,
       UpdateProfiledGoalVelocity(unprofiled_goal_velocity_, outputs_enabled),
       0.0)
          .finished();

  auto accelerator_y =
      (Eigen::Matrix<double, 1, 1>() << input->accelerator_encoder_position())
          .finished();
  accelerator_r_ =
      (Eigen::Matrix<double, 2, 1>() << 0.0, 0.5 * unprofiled_goal_velocity_)
          .finished();
  ...
  auto accelerator_u =
      accelerator_controller_.Update(accelerator_observer_.x())(0, 0);
  ...
  if (!outputs_enabled || unprofiled_goal_velocity_ <= 0) {
    ...
    if (plant_.x()(1, 0) > kMinimalWorkingVelocity &&
  ...
  shooter_observer_.Update(
      (Eigen::Matrix<double, 1, 1>() << shooter_u).finished(), shooter_y);
  accelerator_observer_.Update(
      (Eigen::Matrix<double, 1, 1>() << accelerator_u).finished(),
      accelerator_y);
  plant_.Update((Eigen::Matrix<double, 1, 1>() << shooter_u).finished());

  double absolute_error =
      std::abs(unprofiled_goal_velocity_ - shooter_observer_.x(1));
  ...
  switch (state_) {
    case IDLE:
      ...
      break;
    case SPINUP:
      if (absolute_error < kSpinupVelocityTolerance ||
          encoder_fault_detected_) {
        state_ = AT_GOAL;
      }
      break;
    case AT_GOAL:
      if (absolute_error > kSteadyStateVelocityTolerance &&
          !encoder_fault_detected_) {
        state_ = SPINUP;
      }
      break;
...
double ShooterController::UpdateProfiledGoalVelocity(
    double unprofiled_goal_velocity, bool outputs_enabled) {
  if (outputs_enabled) {
    profiled_goal_velocity_ =
        std::min(profiled_goal_velocity_ + kShooterAcceleration,
                 unprofiled_goal_velocity);
  } else {
    profiled_goal_velocity_ =
        std::max(profiled_goal_velocity_ - kShooterAcceleration, 0.0);
  }
